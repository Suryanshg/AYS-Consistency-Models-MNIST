...
class ConsistencyUNet(nn.Module):
    def __init__(self,
        ...
        self.dconv_up2 = TimeAwareConv(feature_map_dim * 4 + feature_map_dim * 2, feature_map_dim * 2,
                                       time_embedding_dim)
    ...
    def forward(self, x: torch.Tensor, time_step: torch.Tensor) -> torch.Tensor:
        ...
        c_skip, c_out, c_in = self.get_scaling_factors(
            time_step)  # All have shape: (N,)
        c_skip, c_out, c_in = c_skip.view(-1, 1, 1, 1), c_out.view(-1, 1, 1, 1), c_in.view(-1, 1, 1,
                                                                                           1)  # All have shape: (N, 1, 1, 1)
